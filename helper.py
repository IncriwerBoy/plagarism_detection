...
with open('random_forest_model2.pkl' , 'rb') as model:
    rfc2 = pickle.load(model)
...
def pipeline(ansfile, origfile):
    a_text = preprocess_text(ansfile)
    o_text = preprocess_text(origfile)
...
def plag_detection(features):
    prediction = rfc.predict(features)[0]
    return prediction

# def plagType_detection(features, pred1):
#     pred1 = np.array([[pred1]])
#     features2 = np.concatenate((features, pred1), axis=1)
#     prediction2 = rfc2.predict(features2)[0]
#     return prediction2